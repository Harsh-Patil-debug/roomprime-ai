...
export function NivasaEmblemSvg({ className = "size-9" }: { className?: string }) {
  return (
...
export function NivasaLogo({ className = "", showText = true, size = "md" }: NivasaLogoProps) {
  const iconSizeClass = size === "sm" ? "size-7" : size === "lg" ? "size-12" : "size-9";

  return (
    <div className={`flex items-center gap-2.5 select-none ${className}`}>
      <NivasaEmblemSvg className={iconSizeClass} />
      {showText && (
