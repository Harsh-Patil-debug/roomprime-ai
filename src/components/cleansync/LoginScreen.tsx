...
export function LoginScreen() {
  ...
  const [simEmail, setSimEmail] = useState("sanjay.patel@grandpalace.com");
  const [simName, setSimName] = useState("Sanjay Patel");
  const [useSimulated, setUseSimulated] = useState(false);
  const [isGoogleEnabled, setIsGoogleEnabled] = useState(false);

  // 1. Determine local vs tunnel mode reactively on the client
  useEffect(() => {
    const isLocal = typeof window !== "undefined" && 
      (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1");
    
    setIsGoogleEnabled(isGoogleConfigured && isLocal);
  }, []);

  // 2. Load and initialize GSI script
  useEffect(() => {
    ...
    const initGsi = () => {
      try {
        if (window.google?.accounts?.id) {
          ...
          // Re-render button if container is already in the DOM
  ...
  // 3. Callback ref to render the button as soon as the container element mounts
  const googleBtnRef = (el: HTMLDivElement | null) => {
    ...
    const tryRender = () => {
      if (window.google?.accounts?.id) {
        ...
        // Retry shortly if GSI library is still loading
    ...
    <div className="flex min-h-[75vh] items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white border border-[#EBE3D1] shadow-md rounded-2xl p-6 sm:p-8 space-y-6">
        ...
        <div className="space-y-3">
          {isGoogleEnabled && !useSimulated ? (
            <div className="space-y-4 flex flex-col items-center">
              {/* Official Google GSI button target container */}
              <div id="google-signin-btn" ref={googleBtnRef} className="w-full min-h-[44px] flex justify-center" />
              {loading && (
              ...
              <button
                ...
                className="text-[11px] text-[#B5652F] hover:underline font-semibold cursor-pointer mt-1"
              >
                Or, use Developer Bypass Mode (skip Google verification)
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="space-y-3.5">
                <div className="space-y-1.5">
                  <label htmlFor="sim-email" className="text-[10px] font-bold uppercase tracking-wider text-[#736B5E]">
                    Gmail / Email Address
                  </label>
                  <input
                    id="sim-email"
                    ...
                    className="w-full h-10 px-3 rounded-xl border border-[#DDD3BE] bg-transparent text-xs text-[#2A2620] focus:outline-none focus:ring-1 focus:ring-[#B5652F]"
                  />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="sim-name" className="text-[10px] font-bold uppercase tracking-wider text-[#736B5E]">
                    Full Name (Optional)
                  </label>
                  <input
                    id="sim-name"
              ...
              <Button
                onClick={() => loginWithGoogle(simEmail, simName)}
                disabled={loading}
                className="w-full h-11 bg-[#B5652F] hover:bg-[#B5652F]/90 text-white font-semibold text-sm transition-all rounded-xl gap-2 cursor-pointer mt-1"
              >
                {loading ? (
                  ...
                  <svg className="size-4.5 shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.08H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.92l2.85-2.22.81-.6z" fill="currentColor" />
                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.08l3.66 2.84c.87-2.6 3.3-4.54 6.16-4.54z" fill="currentColor" />
                  </svg>
                )}
                <span>{loading ? "Authenticating session..." : "Continue with Google (Demo)"}</span>
              </Button>
              {isGoogleEnabled && (
                <button
                  type="button"
                  onClick={() => setUseSimulated(false)}
                  className="w-full text-center text-[10px] text-muted-foreground hover:underline cursor-pointer font-semibold mt-2"
                >
                  Or, back to official Google Sign-In
                </button>
              )}
            </div>
          )}
